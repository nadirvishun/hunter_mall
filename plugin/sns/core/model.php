...
if (!class_exists('SnsModel')) 
{
	class SnsModel extends PluginModel 
	{
		...
		public function sendMemberUpgradeMessage($openid, $nickname, $oldlevel, $level) 
		{
			...
			$message = str_replace('[旧等级]', $oldlevel['levelname'], $message);
		...
		{
			return str_replace("\n", '<br/>', preg_replace('/\\[EM(\\w+)\\]/', '<img src="../addons/hunter_mall/plugin/sns/static/images/face/${1}.gif" class="emoji" />', $content));
		}
		...
		public function getAvatar($avatar = '') 
		{
			...
			{
				$set = $this->getSet();
				$avatar = ((empty($set['head']) ? '../addons/hunter_mall/plugin/sns/static/images/head.jpg' : tomedia($set['head'])));
			}
			return $avatar;
		}
	}
