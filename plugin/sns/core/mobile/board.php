...
require hunter_mall_PLUGIN . 'sns/core/page_mobile.php';
class Board_EweiShopV2Page extends SnsMobilePage 
{
	...
	public function best() 
	{
		...
		$_W['shopshare'] = array('title' => (!empty($board['share_title']) ? $board['share_title'] : $board['title']), 'imgUrl' => (!empty($board['share_icon']) ? tomedia($board['share_icon']) : tomedia($board['logo'])), 'link' => mobileUrl('sns', array(), true), 'desc' => (!empty($board['share_desc']) ? $board['share_desc'] : $board['desc']));
		include $this->template();
	}
