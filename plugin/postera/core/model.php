...
class PosteraModel extends PluginModel 
{
	...
	public function getQR($poster, $member) 
	{
		...
		if (empty($qr)) 
		{
			...
			$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $scene_id, 'model' => 0, 'name' => 'hunter_mall_POSTERA_QRCODE', 'keyword' => 'hunter_mall_POSTERA', 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/hunter_mall/data/postera/' . $_W['uniacid'] . '/';
		if (!is_dir($path)) 
		{
			load()->func('file');
			mkdirs($path);
