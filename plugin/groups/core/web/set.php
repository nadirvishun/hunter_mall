...
class Set_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'hunter_mall:groups') {
					show_json(0, '关键字已存在!');
				}
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'hunter_mall', ':name' => 'hunter_mall:groups'));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall:groups', 'module' => 'hunter_mall', 'displayorder' => 0, 'status' => 1);
