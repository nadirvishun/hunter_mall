...
class Exhelper_EweiShopV2Page extends PluginWebPage
{
	...
	public function getdata()
	{
		...
		if ($_W['ispost']) {
			$condition = ' o.uniacid = :uniacid and m.uniacid = :uniacid and o.deleted=0 and o.addressid<>0 and isverify = 0 ';
	...
	public function getdatabatch()
	{
		...
		if ($_W['ispost']) {
			...
			if ($status != '') {
				if ($status == 1) {
					$statuscondition = ' and o.status = 1 and (o.is_team = 0 or o.success = 1 ) ';
				}
				 else {
					$statuscondition = ' AND o.status = ' . intval($status);
				}
