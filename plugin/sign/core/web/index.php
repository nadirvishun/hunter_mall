...
class Index_EweiShopV2Page extends PluginWebPage
{
	...
	public function set()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'hunter_mall:sign') {
					show_json(0, '关键字已存在!');
				}
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'hunter_mall:sign'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall:sign', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'hunter_mall', 'title' => $data['title'], 'description' => $data['desc'], 'thumb' => $data['thumb'], 'url' => mobileUrl('sign'));
