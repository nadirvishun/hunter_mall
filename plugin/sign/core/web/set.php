...
class Set_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'hunter_mall签到入口设置'));
			if (!empty($rule)) 
			{
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall签到入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			if (empty($rule)) 
			{
