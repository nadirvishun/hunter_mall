...
if (!class_exists('CommissionModel')) 
{
	class CommissionModel extends PluginModel 
	{
		...
		public function calculate($orderid = 0, $update = true) 
		{
			...
			$goods = pdo_fetchall('select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,' . "\r\n" . '                  g.commission3_rate,g.commission3_pay,g.commission,og.commissions from ' . tablename('ewei_shop_order_goods') . '  og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id = og.goodsid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			if (0 < $set['level']) 
			{
		...
		{
			global $_W;
			$path = IA_ROOT . '/addons/hunter_mall/data/qrcode/' . $_W['uniacid'];
			if (!is_dir($path)) 
			{
				load()->func('file');
				mkdirs($path);
			...
			return $_W['siteroot'] . 'addons/hunter_mall/data/qrcode/' . $_W['uniacid'] . '/' . $file;
		}
		...
		public function createGoodsImage($goods) 
		{
			...
			$path = IA_ROOT . '/addons/hunter_mall/data/poster/' . $_W['uniacid'] . '/';
			if (!is_dir($path)) 
			{
				load()->func('file');
				mkdirs($path);
			...
			{
				set_time_limit(0);
				$font = IA_ROOT . '/addons/hunter_mall/static/fonts/msyh.ttf';
				$target = imagecreatetruecolor(640, 1225);
				$bg = imagecreatefromjpeg(IA_ROOT . '/addons/hunter_mall/plugin/commission/static/images/poster.jpg');
			...
			return $_W['siteroot'] . 'addons/hunter_mall/data/poster/' . $_W['uniacid'] . '/' . $file;
		}
