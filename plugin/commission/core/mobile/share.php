...
require hunter_mall_PLUGIN . 'commission/core/page_mobile.php';
class Share_EweiShopV2Page extends CommissionMobilePage
{
	public function main()
	{
		...
		$qrcode = '';
		$path = $_W['siteroot'] . 'addons/hunter_mall/data/poster/' . $_W['uniacid'] . '/';
		...
		if (!empty($goodsid)) {
			...
			if (!empty($goods)) {
				...
				$qrcode = $path . $md5 . '.jpg';
				if (p('poster') && !is_file(IA_ROOT . 'addons/hunter_mall/data/poster/' . $_W['uniacid'] . '/' . $md5 . '.jpg')) {
					...
					if (!empty($poster)) {
						$md5 = md5(json_encode(array('openid' => $member['openid'], 'goodsid' => $goodsid, 'bg' => $poster['bg'], 'data' => $poster['data'], 'version' => 1)));
						$qrcode = $path . $md5 . '.png';
					}
		...
		if (!$share_goods) {
			...
			if (p('poster') && !is_file(IA_ROOT . 'addons/hunter_mall/data/poster/' . $_W['uniacid'] . '/' . $md5 . '.jpg')) {
				...
				if (!empty($poster)) {
					$md5 = md5(json_encode(array('openid' => $member['openid'], 'goodsid' => '0', 'bg' => $poster['bg'], 'data' => $poster['data'], 'version' => 1)));
					$qrcode = $path . $md5 . '.png';
				}
