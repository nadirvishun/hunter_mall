...
class Level_EweiShopV2Page extends PluginWebPage 
{
	...
	{ 
		...
		$default = array('id' => 'default', 'levelname' => (empty($set['levelname']) ? '默认等级' : $set['levelname']), 'commission1' => $set['commission1'], 'commission2' => $set['commission2'], 'commission3' => $set['commission3'],'level_merch'=>$set['level_merch']);
		...
		if ($_W['ispost']) 
		{
			...
			if (!empty($id)) 
			{
				if ($id == 'default') 
				{
					...
					$set['level_merch']=$data['level_merch'];
					$this->updateSet($set);
