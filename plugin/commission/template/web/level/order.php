...
class Order_EweiShopV2Model 
{
	public function payResult($params) 
	{
		...
		{
			$this->ddd('notify called');
			 $order_id=pdo_fetch('select id from '.tablename('ewei_shop_order').' where ordersn=:ordersn',array(':ordersn'=>$ordersn));
		    $goods=pdo_fetch('select goodsid from '.tablename('ewei_shop_order_goods').' where orderid=:orderid',array(':orderid'=>$order_id['id']));
			 $level_merch=pdo_fetchcolumn('select level_merch  from '.tablename('ewei_shop_goods').' where id=:id',array(':id'=>$goods['goodsid']),0);
			
			 $agents=pdo_fetch('select agentid,agentlevel from '.tablename('ewei_shop_member').' where openid=:openid and isagent=1 and uniacid=:uniacid limit 1',array(':openid'=>$param['openid'],':uniacid'=>$_W['uniacid']));
			 
			if(count($agents)==0){
				//第一次加入分销表时，上线如何获取？
			 pdo_insert(tablename('ewei_shop_member'),array('uniacid'=>$_W['uniacid'],'openid'=>$param['openid'],'isagent'=>1,'agentid'=>//上线id));
			
			 }
			//获取分销商等级
			else{
				$level_merch_now=pdo_fetchcolumn('select level_merch from '.tablename('ewei_shop_commission_level').' where id=:id ',array(':id'=>$agents['agentlevel']));
				
			if(level_merch>level_merch_now){
				 if($level_merch==4){
				//取本客户的上线，并加入到上线的下线中
				//取上线的下线数目
				$num=pdo_fetchcolumn('select count(*)  from '.tablename('ewei_shop_member').' where agentid=:agentid',array(':agentid'=>$agents['agentid']));
				 if($num>=3){
					//上线Level_merch更新为5
					$id=pdo_fetch('select id from '.tablename('ewei_shop_commission_level').' where level_merch=:level_merch',array(':level_merch'=>5));
					pdo_update(tablename('ewei_shop_member'),array('agentlevel'=>$id),array('id'=>$agents['agentid']));
				 }
			 }
			}
	...
	public function ddd($d){
		file_put_contents(IA_ROOT."/addons/hunter_mall/log.txt","\n".date('Y-m-d H:i:s',time())." : ".$d,FILE_APPEND);
	}
}
?>