...
class Set_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'hunter_mall:qa') {
					show_json(0, '关键字已存在！');
				}
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'hunter_mall:qa'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall:qa', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
