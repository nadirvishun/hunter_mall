...
class Index_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function cut($actor_id, $time3, $min_price, $each_time, $total_time, $max_price, $probability_json) 
	{
		...
		{
			$chongzhi = array('now_price' => $max_price, 'bargain_price' => 0);
			pdo_update('ewei_shop_bargain_actor', $chongzhi, array('id' => $actor_id));
			return '砍价异常已重置';
		}
		if ($now_price['now_price'] == $min_price) 
		{
		...
		if ($temp_price <= $min_price) 
		{
			...
			$message_sql = 'SELECT b.* FROM ' . tablename('ewei_shop_bargain_account') . ' a INNER JOIN ' . tablename('ewei_shop_member_message_template') . ' b ON a.end_message =b.id';
			$message_res = pdo_fetch($message_sql);
		...
		if ($cut_price < 0) 
		{
			$message_sql = 'SELECT b.* FROM ' . tablename('ewei_shop_bargain_account') . ' a INNER JOIN ' . tablename('ewei_shop_member_message_template') . ' b ON a.message =b.id';
			$message_res = pdo_fetch($message_sql);
			$now_price['now_price'] += $cut_price;
			$cut_price = -1 * $cut_price;
			if (!empty($message_res['data'])) 
