...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	public function warehouse() 
	{
		...
		$pager = pagination($count['COUNT(*)'], $page, $psize);
		include $this->template();
	}
	public function act() 
	{
	...
	public function react() 
	{
		...
		if (!empty($end_price) || ($end_price === 0)) 
		{
			...
			$rand_arr['rand'] = $rand;
			$rand_json = json_encode($rand_arr);
			$data = array('myself' => $myself, 'account_id' => $_W['uniacid'], 'goods_id' => $this_goods['id'], 'end_price' => $end_price, 'start_time' => $act_time['start'], 'end_time' => $act_time['end'], 'status' => 0, 'type' => $dispaly, 'user_set' => $user_set_json, 'rule' => $rule, 'act_times' => '0', 'mode' => $order_set, 'total_time' => $total_time, 'each_time' => $each_time, 'time_limit' => $time_limit, 'probability' => $rand_json, 'custom' => $custom, 'maximum' => $maximum);
	...
	public function messageset() 
	{
		...
		if ($_W['ispost']) 
		{
			$message = intval($_GPC['message']);
			$end_message = intval($_GPC['end_message']);
			pdo_update('ewei_shop_bargain_account', array('message' => $message, 'end_message' => $end_message), array('id' => $_W['uniacid']));
			show_json(1, '保存成功');
			return;
		}
		include $this->template();
