...
class Index_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (($item['keyword2'] != $data['keyword2']) && !empty($keyword)) {
				if ($keyword['name'] != 'hunter_mall:poster:' . $data['type']) {
					show_json(0, '关键字已存在!');
				}
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'hunter_mall', ':name' => 'hunter_mall:poster:' . $data['type']));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall:poster:' . $data['type'], 'module' => 'hunter_mall', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'hunter_mall', 'content' => ($data['type'] == 3 ? '^' . trim($data['keyword2']) . '\\+*[0-9]{1,}$' : trim($data['keyword2'])), 'type' => ($data['type'] == 3 ? 3 : 1), 'displayorder' => 0, 'status' => 1);
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'hunter_mall', ':name' => 'hunter_mall:poster:auto'));

			if (empty($ruleauto)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall:poster:auto', 'module' => 'hunter_mall', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'hunter_mall', 'content' => 'hunter_mall_POSTER', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
	...
	public function clear()
	{
		...
		global $_GPC;
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/hunter_mall/data/poster/' . $_W['uniacid']);
		@rmdirs(IA_ROOT . '/addons/hunter_mall/data/qrcode/' . $_W['uniacid']);
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
