...
class PosterModel extends PluginModel
{
	...
	public function getQR($poster, $member, $goodsid = 0)
	{
		...
		if ($poster['type'] == 2) {
			...
			if (empty($qr) || empty($qr['scenestr'])) {
				...
				$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'scene_str' => $barcode['action_info']['scene']['scene_str'], 'model' => 2, 'name' => 'hunter_mall_POSTER_QRCODE', 'keyword' => 'hunter_mall_POSTER', 'expire' => 0, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
	...
	public function createPoster($poster, $member, $qr, $upload = true)
	{
		...
		$img = $_W['siteroot'] . 'addons/hunter_mall/data/poster/' . $_W['uniacid'] . '/' . $file;
