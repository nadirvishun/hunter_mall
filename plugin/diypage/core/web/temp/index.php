...
class Index_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$category = pdo_fetchall('SELECT id, name FROM ' . tablename('ewei_shop_diypage_template_category') . ' WHERE uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		if ($_W['ispost']) {
			...
			$item = pdo_fetch('SELECT id, name, uniacid FROM ' . tablename('ewei_shop_diypage_template') . ' WHERE id=:id and (uniacid=:uniacid or uniacid=0) ', array(':uniacid' => $_W['uniacid'], ':id' => $tid));
			...
			if (!empty($item)) {
				if (empty($item['uniacid'])) {
					...
					pdo_update('ewei_shop_diypage_template', array('deleted' => 1), array('id' => $tid));
				}
				 else {
