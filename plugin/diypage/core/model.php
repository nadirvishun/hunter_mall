...
class DiypageModel extends PluginModel 
{
	...
	public function getPageList($type = 'allpage', $condition = NULL, $page = 0) 
	{
		...
		$list = pdo_fetchall('select id, `name`, `type`, createtime, lastedittime, keyword from ' . tablename('ewei_shop_diypage') . ' where uniacid=:uniacid ' . $c . ' order by type desc, id desc ' . $limit, array(':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_diypage') . ' where uniacid=:uniacid ' . $c, array(':uniacid' => $_W['uniacid']));
				...
				foreach ($page['data']['items'] as $itemid => &$item ) 
				{
					if ($item['id'] == 'goods') 
					{
						if ($item['params']['goodsdata'] == '0') 
						{
							if (!empty($item['data']) && is_array($item['data'])) 
							{
								...
								if (!empty($goodsids) && is_array($goodsids)) 
								{
									...
									$goods = pdo_fetchall('select id, title, thumb, minprice, total, showlevels, showgroups, bargain from ' . tablename('ewei_shop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									if (!empty($goods) && is_array($goods)) 
									{
										foreach ($goodsids as $goodsid ) 
										{
											foreach ($goods as $index => $good ) 
											{
												if ($good['id'] == $goodsid) 
												{
													$showgoods = (($mobile ? m('goods')->visit($good, $this->member) : true));
													if ($showgoods) 
													{
						...
						else if ($item['params']['goodsdata'] == '1') 
						{
							...
							{
								$category = pdo_fetch('select id,name from ' . tablename('ewei_shop_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								if (!empty($category)) 
								{
					...
					if ($item['id'] == 'goods') 
					{
						if ($item['params']['goodsdata'] == '1') 
						{
							...
							if (!empty($cateid)) 
							{
								...
								$goodslist = m('goods')->getList(array('cate' => $cateid, 'pagesize' => $limit, 'page' => 1, 'order' => $orderby));
						...
						else if ($item['params']['goodsdata'] == '2') 
						{
							...
							if (!empty($group) && !empty($group['goodsids'])) 
							{
								...
								if (!empty($goodssort)) 
								{
									if ($goodssort == 1) 
									{
										$orderby = ' order by sales desc, displayorder desc';
									}
									else if ($goodssort == 2) 
									{
	...
	public function savePage($id, $data, $update = true) 
	{
		...
		if (!empty($keyword) && $update) 
		{
			...
			if (!empty($result)) 
			{
				if ($result['name'] != 'hunter_mall:diypage:' . $id) 
				{
					show_json(0, '关键字已存在！');
				}
			}
		...
		if (!empty($keyword) && $update) 
		{
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'hunter_mall', ':name' => 'hunter_mall:diypage:' . $id));
			if (!empty($rule)) 
			{
			...
			{
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'hunter_mall:diypage:' . $id, 'module' => 'hunter_mall', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'hunter_mall', 'content' => $keyword, 'type' => 1, 'displayorder' => 0, 'status' => 1);
	...
	public function delPage($id) 
	{
		...
		foreach ($items as $item ) 
		{
			...
			$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $item['keyword'], ':module' => 'hunter_mall', ':uniacid' => $_W['uniacid']));
			if (!empty($keyword)) 
			{
	...
	public function setShare($page) 
	{
		...
		$urlpage = NULL;
		$urlparm = array();
		$_W['shopshare'] = array('title' => $_W['shopset']['shop']['name'], 'imgUrl' => tomedia($_W['shopset']['shop']['logo']), 'desc' => $_W['shopset']['shop']['description'], 'link' => mobileUrl(NULL, NULL, true));
		if ($page['type'] == 1) 
		{
			$_W['shopshare']['link'] = mobileUrl('diypage', array('id' => $page['id']), true);
			$urlpage = 'diypage';
			$urlparm['id'] = $page['id'];
		}
	...
	public function detailPage($pageid = 0) 
	{
		...
		return array('background' => $background, 'followbar' => $followbar, 'tab' => $detail_tab, 'navbar' => $detail_navbar, 'diynavbar' => $navbar, 'comment' => $detail_comment, 'diylayer' => $page['data']['page']['diylayer'], 'items' => $pageitems);
	}
