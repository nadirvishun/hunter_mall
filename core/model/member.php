...
class Member_EweiShopV2Model
{
	...
	public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array())
	{
		...
		if (!empty($uid)) {
			...
			$data = array('uid' => $uid, 'credittype' => $credittype, 'uniacid' => $_W['uniacid'], 'num' => $credits, 'createtime' => TIMESTAMP, 'module' => 'hunter_mall', 'operator' => intval($log[0]), 'remark' => $log[1]);
	...
	public function checkMember()
	{
		...
		$openid = $_W['openid'];
		if (($_W['routes'] == 'order.pay_alipay') || ($_W['routes'] == 'order.pay_alipay.recharge_complete') || ($_W['routes'] == 'order.pay_alipay.complete')) {
			return NULL;
		}
		...
		if (empty($openid) && !hunter_mall_DEBUG) {
	...
	public function checkMemberSNS($sns)
	{
		...
		$data = array('openid' => $userinfo['openid'], 'nickname' => $userinfo['nickname'], 'avatar' => $userinfo['headimgurl'], 'province' => $userinfo['province'], 'city' => $userinfo['city'], 'gender' => $userinfo['sex'], 'uniacid' => $_W['uniacid'], 'comefrom' => 'h5app_sns_' . $sns);
