...
class Common_EweiShopV2Model 
{
	...
	public function alipay_build($params, $alipay = array(), $type = 0, $openid = '') 
	{
		...
		if (empty($type)) 
		{
			$set['notify_url'] = $_W['siteroot'] . 'addons/hunter_mall/payment/alipay/notify.php';
	...
	protected function publicAliPay($params = array()) 
	{
		...
		$response = ihttp_request('https://openapi.alipay.com/gateway.do', $public);
		$result = json_decode(iconv('gbk', 'utf-8', $response['content']), true);
		return $result;
	}
	...
	public function AliPayBarcode($params, $config) 
	{
		...
		$biz_content['seller_id'] = $params['seller_id'];
		...
		$config['notify_url'] = $_W['siteroot'] . 'addons/hunter_mall/payment/alipay/notify.php';
		...
		return error($result['alipay_trade_pay_response']['code'], $result['alipay_trade_pay_response']['msg']);
	}
	public function AliPayQuery($out_trade_no, $config) 
	{
		...
		return error($result['alipay_trade_query_response']['code'], $result['alipay_trade_query_response']['msg']);
	}
	...
	public function wechat_child_build($params, $wechat, $type = 0) 
	{
		...
		$package['nonce_str'] = random(32);
		$package['body'] = $params['title'];
		$package['device_info'] = ((isset($params['device_info']) ? 'hunter_mall:' . $params['device_info'] : 'hunter_mall'));
		...
		$package['notify_url'] = ((empty($params['notify_url']) ? $_W['siteroot'] . 'addons/hunter_mall/payment/wechat/notify.php' : $params['notify_url']));
		$package['trade_type'] = 'JSAPI';
	...
	public function sendredpack($params, $wechat) 
	{
		...
		if (is_array($wechat['certs'])) 
		{
			...
			$certfile = IA_ROOT . '/addons/hunter_mall/cert/' . random(128);
			file_put_contents($certfile, $wechat['certs']['cert']);
			$keyfile = IA_ROOT . '/addons/hunter_mall/cert/' . random(128);
			file_put_contents($keyfile, $wechat['certs']['key']);
			$rootfile = IA_ROOT . '/addons/hunter_mall/cert/' . random(128);
	...
	public function wechat_order_query($out_trade_no, $money, $wechat, $app = false) 
	{
		$package = array();
		$package['appid'] = $wechat['appid'];
		$package['mch_id'] = $wechat['mchid'];
	...
	public function paylog($log = '') 
	{
		...
		if (!empty($openpaylog)) 
		{
			$path = IA_ROOT . '/addons/hunter_mall/data/paylog/' . $_W['uniacid'] . '/' . date('Ymd');
			if (!is_dir($path)) 
			{
				load()->func('file');
				@mkdirs($path, '0777');
	...
	{
		$file = IA_ROOT . '/addons/hunter_mall/static/js/dist/area/Area.xml';
