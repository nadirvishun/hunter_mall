...
class Route_EweiShopV2Model 
{
	public function run($isweb = true) 
	{
		...
		if ($isweb) 
		{
			...
			require_once hunter_mall_CORE . 'inc/page_web_com.php';
		}
		else 
		{
			require_once hunter_mall_CORE . 'inc/page_mobile.php';
			require_once hunter_mall_CORE . 'inc/page_mobile_login.php';
		}
		$r = str_replace('//', '/', trim($_GPC['r'], '/'));
		$routes = explode('.', $r);
		$segs = count($routes);
		$method = 'main';
		$root = (($isweb ? hunter_mall_CORE_WEB : hunter_mall_CORE_MOBILE));
		$isMerch = false;
		...
		{
			$isplugin = !empty($r) && is_dir(hunter_mall_PLUGIN . $routes[0]);
		}
		if ($isplugin) 
		{
			if ($isweb) 
			{
				require_once hunter_mall_CORE . 'inc/page_web_plugin.php';
			}
			else 
			{
				require_once hunter_mall_CORE . 'inc/page_mobile_plugin.php';
				require_once hunter_mall_CORE . 'inc/page_mobile_plugin_login.php';
				require_once hunter_mall_CORE . 'inc/page_mobile_plugin_pf.php';
			}
			$_W['plugin'] = $routes[0];
			$root = hunter_mall_PLUGIN . $routes[0] . '/core/' . (($isweb ? 'web' : 'mobile')) . '/';
			if ($isMerch) 
			{
				$_W['plugin'] = 'merch';
				$root = hunter_mall_PLUGIN . 'merch/core/web/manage/';
			}
			else 
			{
		...
		{
			require_once hunter_mall_CORE . 'inc/page_system.php';
		}
		switch ($segs) 
		{
