...
class Account_EweiShopV2Model 
{
	...
	public function checkOpenid() 
	{
		global $_W;
		global $_GPC;
		$key = '__hunter_mall_member_session_' . $_W['uniacid'];
		if (isset($_GPC[$key])) 
		{
			...
			if (is_array($session)) 
			{
				...
				if (is_array($member) && ($session['hunter_mall_member_hash'] == md5($member['pwd'] . $member['salt']))) 
				{
					$GLOBALS['_W']['hunter_mall_member_hash'] = md5($member['pwd'] . $member['salt']);
					$GLOBALS['_W']['hunter_mall_member'] = $member;
	...
	public function setLogin($member) 
	{
		...
		if (!empty($member)) 
		{
			$member['hunter_mall_member_hash'] = md5($member['pwd'] . $member['salt']);
			$key = '__hunter_mall_member_session_' . $_W['uniacid'];
