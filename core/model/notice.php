...
class Notice_EweiShopV2Model 
{
	protected function getUrl($do, $query = NULL) 
	{
		...
		if (strexists($url, '/addons/hunter_mall/')) 
		{
			$url = str_replace('/addons/hunter_mall/', '/', $url);
		}
	...
	public function sendOrderMessage($orderid = '0', $delRefund = false) 
	{
		...
		else if ($order['status'] == 1) 
		{
			...
			else 
			{
				...
				$buyerinfo_mobile = $carrier['carrier_mobile'];
				goto label298;
				label298: goto label308;
				label308: goto label321;
				label321: goto label333;
				label333: goto label336;
				label336: $datas = array( array('name' => '商城名称', 'value' => $_W['shopset']['shop']['name']), array('name' => '粉丝昵称', 'value' => $member['nickname']), array('name' => '订单号', 'value' => $order['ordersn']), array('name' => '订单金额', 'value' => $order['price']), array('name' => '运费', 'value' => $order['dispatchprice']), array('name' => '商品详情', 'value' => $goods), array('name' => '快递公司', 'value' => $order['expresscom']), array('name' => '快递单号', 'value' => $order['expresssn']), array('name' => '购买者姓名', 'value' => $buyerinfo_name), array('name' => '购买者电话', 'value' => $buyerinfo_mobile), array('name' => '收货地址', 'value' => $addressinfo), array('name' => '下单时间', 'value' => date('Y-m-d H:i', $order['createtime'])), array('name' => '支付时间', 'value' => date('Y-m-d H:i', $order['paytime'])), array('name' => '发货时间', 'value' => date('Y-m-d H:i', $order['sendtime'])), array('name' => '收货时间', 'value' => date('Y-m-d H:i', $order['finishtime'])), array('name' => '门店', 'value' => ''), array('name' => '门店地址', 'value' => ''), array('name' => '门店联系人', 'value' => $store['realname'] . '/' . ''), array('name' => '门店营业时间', 'value' => ''), array('name' => '虚拟物品自动发货内容', 'value' => $order['virtualsend_info']), array('name' => '虚拟卡密自动发货内容', 'value' => $order['virtual_str']), array('name' => '自提码', 'value' => $order['verifycode']) );
				$usernotice = unserialize($member['noticeset']);
				$usernotice = array();
				$set = m('common')->getSysset();
				$shop = $set['shop'];
				...
				$datas[] = array('name' => '售后类型', 'value' => $r_type[$refund['rtype']]);
				goto label430;
				label430: $datas[] = array('name' => '申请金额', 'value' => $refund['applyprice']);
				$datas[] = array('name' => '退款金额', 'value' => $refund['price']);
				$datas[] = array('name' => '换货快递公司', 'value' => $refund['rexpresscom']);
				$datas[] = array('name' => '换货快递单号', 'value' => $refund['rexpresssn']);
				return NULL;
				goto label471;
				label471: $msg = array( 'first' => array('value' => '您的' . $r_type[$refund['rtype']] . '申请已经提交！', 'color' => '#4a5077'), 'orderProductPrice' => array('title' => '退款金额', 'value' => '¥' . $refund['applyprice'] . '元', 'color' => '#4a5077'), 'orderProductName' => array('title' => '商品详情', 'value' => $goods . $orderpricestr, 'color' => '#4a5077'), 'orderName' => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#4a5077'), 'remark' => array('value' => "\r\n" . '等待商家确认' . $r_type[$refund['rtype']] . '信息！', 'color' => '#4a5077') );
				$this->sendNotice(array('openid' => $openid, 'tag' => 'refund', 'default' => $msg, 'url' => $url, 'datas' => $datas));
				com_run('sms::callsms', array('tag' => 'refund', 'datas' => $datas, 'mobile' => $member['mobile']));
				return NULL;
				return NULL;
				...
				$datas[] = array('name' => '退货地址', 'value' => $refundaddressinfo);
				goto label583;
				label583: $msg = array( 'first' => array('value' => '您的' . $r_type[$refund['rtype']] . '申请已经通过！', 'color' => '#4a5077'), 'keyword1' => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#4a5077'), 'keyword2' => array('title' => '当前进度', 'value' => '您的' . $r_type[$refund['rtype']] . '申请已经通过！', 'color' => '#4a5077'), 'keyword3' => array('title' => '商品名称', 'value' => $goods . $orderpricestr, 'color' => '#4a5077'), 'keyword4' => array('title' => '退款金额', 'value' => '¥' . $refund['applyprice'] . '元', 'color' => '#4a5077'), 'remark' => array('value' => "\r\n" . '请您根据商家提供的退货地址将商品寄回！' . $refund_address . '', 'color' => '#4a5077') );
				$this->sendNotice(array('openid' => $openid, 'tag' => 'refunding', 'default' => $msg, 'url' => $url, 'datas' => $datas));
				com_run('sms::callsms', array('tag' => 'refunding', 'datas' => $datas, 'mobile' => $member['mobile']));
				return NULL;
				return NULL;
				...
				return NULL;
				goto label689;
				label689: $msg = array( 'first' => array('value' => '您的换货物品已经发货！', 'color' => '#4a5077'), 'keyword1' => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#4a5077'), 'keyword2' => array('title' => '当前进度', 'value' => '您的换货物品已经发货！快递公司: ' . $refund['rexpresscom'] . ' 快递单号: ' . $refund['rexpresssn'], 'color' => '#4a5077'), 'keyword3' => array('title' => '商品名称', 'value' => $goods . $orderpricestr, 'color' => '#4a5077'), 'keyword4' => array('title' => '退款金额', 'value' => '¥' . $refund['applyprice'] . '元', 'color' => '#4a5077'), 'remark' => array('value' => "\r\n" . '我们正加速送到您的手上，请您耐心等候。', 'color' => '#4a5077') );
				$this->sendNotice(array('openid' => $openid, 'tag' => 'refunding', 'default' => $msg, 'url' => $url, 'datas' => $datas));
				com_run('sms::callsms', array('tag' => 'refunding', 'datas' => $datas, 'mobile' => $member['mobile']));
				return NULL;
				return NULL;
