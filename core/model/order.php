...
class Order_EweiShopV2Model 
{
	public function payResult($params) 
	{
		...
		if ($params['from'] == 'return') 
		{
			...
			$this->ddd("return called");
			//自定义代码
			 $order_id=pdo_fetch('select id from '.tablename('ewei_shop_order').' where ordersn=:ordersn and uniacid=:uni',array(':ordersn'=>$ordersn,':uni'=>$_W['uniacid']));
			...
			$i=0;
			$this->ddd('openid:  '.$new_openid);
			do{
			$openid=$new_openid;
			$agents=pdo_fetch('select id,agentid,agentlevel,status from '.tablename('ewei_shop_member').' where openid=:openid and uniacid=:uniacid limit 1',array(':openid'=>$openid,':uniacid'=>$_W['uniacid']));
			
			
			$level_merch_this=pdo_fetchcolumn('select level_merch from '.tablename('ewei_shop_commission_level').' where id=:agentlevel limit 1',array(':agentlevel'=>$agents['agentlevel']));
			
			$new_openid=$agents['agentid'];
