...
class Page extends WeModuleSite 
{
	public function runTasks() 
	{
		...
		if (($lasttime + $interval) <= $current) 
		{
			...
			ihttp_request(hunter_mall_TASK_URL . 'order/receive.php', NULL, NULL, 1);
		}
		...
		if (($lasttime + $interval) <= $current) 
		{
			...
			ihttp_request(hunter_mall_TASK_URL . 'order/close.php', NULL, NULL, 1);
		}
		if (com('coupon')) 
		{
			...
			if (($lasttime + $interval) <= $current) 
			{
				...
				ihttp_request(hunter_mall_TASK_URL . 'coupon/back.php', NULL, NULL, 1);
			}
		}
		if (p('groups')) 
		{
			...
			if (($groups_order_lasttime + $groups_order_interval) <= $groups_order_current) 
			{
				...
				ihttp_request($_W['siteroot'] . 'addons/hunter_mall/plugin/groups/task/order.php', NULL, NULL, 1);
			}
			...
			if (($groups_team_lasttime + $groups_team_interval) <= $groups_team_current) 
			{
				...
				ihttp_request($_W['siteroot'] . 'addons/hunter_mall/plugin/groups/task/refund.php', NULL, NULL, 1);
			}
			...
			if (($groups_receive_lasttime + $groups_receive_interval) <= $groups_receive_current) 
			{
				...
				ihttp_request($_W['siteroot'] . 'addons/hunter_mall/plugin/groups/task/receive.php', NULL, NULL, 1);
			}
		}
		exit('run finished.');
	}
	public function template($filename = '', $type = TEMPLATE_INCLUDEPATH, $account = false) 
	{
		...
		$name = 'hunter_mall';
		$moduleroot = IA_ROOT . '/addons/hunter_mall';
		if (defined('IN_SYS')) 
		{
