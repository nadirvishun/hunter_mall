...
class MobilePage extends Page 
{
	...
	public function __construct() 
	{
		...
		else 
		{
			...
			{
				$_W['openid'] = 'otNFxuOh8MWAIewTiZ_tpLdiSKc0';
			}
		}
	...
	public function followBar($diypage = false) 
	{
		...
		if ((!$followed && ($memberid != $mid)) || (!empty($diyfollowbar['params']['showtype']) && !empty($diyfollowbar['params']['isopen']))) 
		{
			...
			{
				if (!empty($_SESSION[hunter_mall_PREFIX . '_shareid']) && ($_SESSION[hunter_mall_PREFIX . '_shareid'] == $mid)) 
				{
					$mid = $_SESSION[hunter_mall_PREFIX . '_shareid'];
				}
