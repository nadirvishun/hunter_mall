...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'hunter_mall', ':tid' => $order['ordersn']));
		...
		if (empty($log)) 
		{
			$log = array('uniacid' => $uniacid, 'openid' => $member['uid'], 'module' => 'hunter_mall', 'tid' => $order['ordersn'], 'fee' => $order['price'], 'status' => 0);
	...
	public function orderstatus() 
	{
		...
		{
			@session_start();
			$_SESSION[hunter_mall_PREFIX . '_order_pay_complete'] = 1;
			show_json(1);
		}
		show_json(0);
	}
	...
	{
		@session_start();
		if (!isset($_SESSION[hunter_mall_PREFIX . '_order_pay_complete'])) 
		{
			header('location: ' . mobileUrl('order'));
			exit();
		}
		unset($_SESSION[hunter_mall_PREFIX . '_order_pay_complete']);
		global $_W;
		global $_GPC;
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
