...
class Upgrade_EweiShopV2Page extends SystemPage
{
	public function main()
	{
		global $_W;
		$auth = get_auth();
		$versionfile = IA_ROOT . '/addons/hunter_mall/version.php';
		$updatedate = date('Y-m-d H:i', filemtime($versionfile));
		$version = hunter_mall_VERSION;
		$release = hunter_mall_RELEASE;
		$domain = trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/')));
		$ip = gethostbyname($_SERVER['HTTP_HOST']);
		$setting = setting_load('site');
		$id = ((isset($setting['site']['key']) ? $setting['site']['key'] : '0'));
		load()->func('communication');
		$resp = ihttp_request(hunter_mall_AUTH_URL, array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain));
	...
	public function check()
	{
		...
		set_time_limit(0);
		$auth = get_auth();
		$version = ((defined('hunter_mall_VERSION') ? hunter_mall_VERSION : '2.0.0'));
		$release = ((defined('hunter_mall_RELEASE') ? hunter_mall_RELEASE : '201605010000'));
		$resp = ihttp_post(hunter_mall_AUTH_URL . 'check', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
		$result = @json_decode($resp['content'], true);
			...
			if ($result['status'] == 1) {
				...
				if (!empty($upgrade['files'])) {
					foreach ($upgrade['files'] as $file ) {
						$entry = hunter_mall_PATH . $file['path'];
		...
		if (is_file(hunter_mall_PATH . 'tmp')) {
			@unlink(hunter_mall_PATH . 'tmp');
		}
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_post(hunter_mall_AUTH_URL . 'download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
				...
				if (is_array($ret)) {
					...
					if (!is_dir(hunter_mall_PATH . $dirpath)) {
						mkdirs(hunter_mall_PATH . $dirpath);
						@chmod(hunter_mall_PATH . $dirpath, 511);
					}


					$content = base64_decode($ret['content']);
					file_put_contents(hunter_mall_PATH . $path, $content);

					if (isset($ret['path1'])) {
						$path1 = $ret['path1'];
						$dirpath1 = dirname($path1);

						if (!is_dir(hunter_mall_PATH . $dirpath1)) {
							mkdirs(hunter_mall_PATH . $dirpath1);
							@chmod(hunter_mall_PATH . $dirpath1, 511);
						}


						$content1 = base64_decode($ret['content1']);
						file_put_contents(hunter_mall_PATH . $path1, $content1);
					}
					...
					if (isset($ret['path2'])) {
						...
						if (!is_dir(hunter_mall_PATH . $dirpath2)) {
							mkdirs(hunter_mall_PATH . $dirpath2);
							@chmod(hunter_mall_PATH . $dirpath2, 511);
						}


						$content2 = base64_decode($ret['content2']);
						file_put_contents(hunter_mall_PATH . $path2, $content2);
					}
		...
		if ($action == 'upgrade') {
			...
			if (!empty($update)) {
				$updatepath = hunter_mall_PATH . 'tmp/';
	...
	{
		load()->func('file');
		file_put_contents(hunter_mall_PATH . 'version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'hunter_mall_VERSION\')) {define(\'hunter_mall_VERSION\', \'' . $version . '\');}if(!defined(\'hunter_mall_RELEASE\')) {define(\'hunter_mall_RELEASE\', \'' . $release . '\');}');
		cache_delete('cloud:modules:upgradev2');
		$time = time();
		global $my_scenfiles;
		my_scandir(IA_ROOT . '/addons/hunter_mall');

		foreach ($my_scenfiles as $file ) {
			if (!strexists($file, '/hunter_mall/data/') && !strexists($file, 'version.php')) {
				@touch($file, $time);
			}

		}

		rmdirs(IA_ROOT . '/addons/hunter_mall/tmp');
	}

	public function checkversion()
	{
		file_put_contents(IA_ROOT . '/addons/hunter_mall/version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'hunter_mall_VERSION\')) {define(\'hunter_mall_VERSION\', \'2.0.0\');}if(!defined(\'hunter_mall_RELEASE\')) {define(\'hunter_mall_RELEASE\', \'201605010000\');}');
