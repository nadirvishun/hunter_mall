...
class Index_EweiShopV2Page extends ComWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$key = 'hunter_mall:com:coupon:' . $id;
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'hunter_mall', ':name' => $key));
			if (!empty($data['pwdkey2'])) 
			{
				if (empty($rule)) 
				{
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => $key, 'module' => 'hunter_mall', 'displayorder' => 0, 'status' => $data['pwdopen']);
					pdo_insert('rule', $rule_data);
					$rid = pdo_insertid();
					$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'hunter_mall', 'content' => $data['pwdkey2'], 'type' => 1, 'displayorder' => 0, 'status' => $data['pwdopen']);
