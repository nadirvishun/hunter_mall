...
class Index_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		if (!empty($item['giftgoodsid'])) 
		{
			...
			if ($giftid) 
			{
				...
				{
					$gift[$key] = pdo_fetch('select id,title,thumb from ' . tablename('ewei_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 ');
				}
			}
		}
		if ($_W['ispost']) 
	...
	public function querygoods() 
	{
		...
		$pager = pagination($total, $pindex, $psize, '', array('before' => 5, 'after' => 4, 'ajaxcallback' => 'select_page'));
