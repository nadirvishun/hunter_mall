...
class Qiniu_EweiShopV2ComModel extends ComModel 
{
	public function save($url, $config = NULL, $enforce = false) 
	{
		...
		{
			$url = str_replace('../addons/hunter_mall', 'addons/hunter_mall', $url);
		}
		if (!strexists($url, 'addons/hunter_mall')) 
		{
			$oldurl = $url;
		...
		if (!$outlinkEnforce) 
		{
			...
			else if (strexists($url, 'addons/hunter_mall')) 
			{
				$url = IA_ROOT . '/' . $url;
			}
			else 
		...
		if ($outlinkEnforce) 
		{
			$local = ATTACHMENT_ROOT . 'hunter_mall_temp/';
			load()->func('file');
			if (!is_dir($local)) 
			{
				mkdirs($local);
	...
	public function getConfig() 
	{
		...
		{
			$path = IA_ROOT . '/addons/hunter_mall/data/global';
